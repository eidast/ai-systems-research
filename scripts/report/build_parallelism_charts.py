...
if len(sys.argv) != 3:
    print('usage: build_parallelism_charts.py <summary-json> <output-dir>', file=sys.stderr)
    sys.exit(1)
...
bar('par-parallel-reduction — mean wall time', wall, langs, 'Milliseconds', 'par-parallel-reduction-wall-time.svg', '#7c3aed')
bar('par-parallel-reduction — mean peak memory', mem, langs, 'MB', 'par-parallel-reduction-memory.svg', '#059669')
print(out_dir)
